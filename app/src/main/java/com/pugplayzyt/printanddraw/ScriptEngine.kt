...
object DrawScriptEngine {
        ...
        suspend fun execute(statements: List<Stmt>) {
            for (statement in statements) {
                when (statement) {
                    ...
                    is Stmt.Pen -> { penDown = statement.down; tick() }
                    is Stmt.Speed -> { speed = Expression(statement.expression, env).value().coerceIn(0.1, 1000.0); tick() }
                    is Stmt.Width -> {
                        currentWidth = Expression(statement.expression, env).value().coerceIn(MIN_PEN_WIDTH, MAX_PEN_WIDTH).toFloat()
                        tick()
                    }
                    is Stmt.Color -> {
                        ...
                        currentColor = ((255L shl 24) or (red.toLong() shl 16) or (green.toLong() shl 8) or blue.toLong()).toInt()
                        tick()
                    }
                    is Stmt.Position -> {
                        val next = bounded(Expression(statement.x, env).value(), Expression(statement.y, env).value())
                        if (penDown && next != pen) emit(Segment(pen, next, currentColor, currentWidth))
                        pen = next
                        tick()
                    ...
                    is Stmt.If -> if (Condition(statement.condition, env).value()) execute(statement.body)
                    Stmt.RegisterFrameSystem -> { frameSystemRegistered = true; tick() }
                    is Stmt.FrameRate -> {
                        if (!frameSystemRegistered) error("Register the frame system before setting frame rate")
                        frameRate = Expression(statement.expression, env).value().coerceIn(MIN_FRAME_RATE, MAX_FRAME_RATE)
                        tick()
                    }
                    is Stmt.GenerateFrames -> {
                        ...
                        val frameCount = Expression(statement.count, env).value().roundToInt().coerceIn(0, MAX_FRAMES)
                        ...
                        for (frameNumber in 1..frameCount) {
                            ...
                            if (frameNumber < frameCount) delay((1000.0 / frameRate).toLong().coerceAtLeast(1L))
                        }
    ...
    private class Parser(source: String) {
        private val lines = source.lineSequence().map { it.substringBefore('#').trim() }.filter { it.isNotEmpty() }.toList()
        private var index = 0
            ...
            while (index < lines.size) {
                ...
                when {
                    ...
                    line.startsWith("repeat ") && line.endsWith("{") -> {
                        val count = line.removePrefix("repeat ").removeSuffix("{").trim(); requireText(count, "repeat count")
                        index++; out += Stmt.Repeat(count, parseBlock(root = false))
                    }
                    line.startsWith("if ") && line.endsWith("{") -> {
                        val condition = line.removePrefix("if ").removeSuffix("{").trim(); requireText(condition, "if condition")
                        index++; out += Stmt.If(condition, parseBlock(root = false))
                    }
                    ...
                    line.startsWith("speed ") -> { out += Stmt.Speed(line.removePrefix("speed ").trim()); index++ }
                    line.startsWith("pen width ") -> { val e = line.removePrefix("pen width ").trim(); requireText(e, "pen width"); out += Stmt.Width(e); index++ }
                    line.startsWith("width ") -> { val e = line.removePrefix("width ").trim(); requireText(e, "pen width"); out += Stmt.Width(e); index++ }
                    line.startsWith("pen color ") -> { out += parseColor(line.removePrefix("pen color ")); index++ }
                    line.startsWith("pen colour ") -> { out += parseColor(line.removePrefix("pen colour ")); index++ }
                    line.startsWith("color ") -> { out += parseColor(line.removePrefix("color ")); index++ }
                    line.startsWith("colour ") -> { out += parseColor(line.removePrefix("colour ")); index++ }
                    line.startsWith("pen position ") -> { out += parsePosition(line.removePrefix("pen position ")); index++ }
                    line.startsWith("pen pos ") -> { out += parsePosition(line.removePrefix("pen pos ")); index++ }
                    line.startsWith("pos ") -> { out += parsePosition(line.removePrefix("pos ")); index++ }
                    line.startsWith("let ") -> { out += parseAssignment(line.removePrefix("let ")); index++ }
                    line.startsWith("set ") -> { out += parseAssignment(line.removePrefix("set ")); index++ }
        ...
        private fun generatedFrameCount(line: String): String? {
            Regex("""generate\s+(.+?)\s+frames\s*\{""").matchEntire(line)?.let { return it.groupValues[1].trim() }
            Regex("""generate\s+frames\s+(.+?)\s*\{""").matchEntire(line)?.let { return it.groupValues[1].trim() }
            Regex("""frames\s+(.+?)\s*\{""").matchEntire(line)?.let { return it.groupValues[1].trim() }
            return null
        }
        ...
        private fun parseAssignment(text: String): Stmt.Assign {
            ...
            return Stmt.Assign(name, text.substring(eq + 1).trim())
        }
        ...
        private fun parsePosition(text: String): Stmt.Position {
            ...
            requireText(parts[0], "x position"); requireText(parts[1], "y position")
        ...
        private fun parseColor(text: String): Stmt.Color {
            ...
            requireText(parts[0], "red channel"); requireText(parts[1], "green channel"); requireText(parts[2], "blue channel")
        ...
        private fun splitTopLevelCommas(text: String): List<String> {
            ...
            parts += text.substring(start).trim(); return parts
        }

        private fun requireText(value: String, name: String) { if (value.isBlank()) error("Missing $name") }
    }
    ...
    private class Condition(private val text: String, private val env: Map<String, Double>) {
        ...
        private fun evaluate(raw: String): Boolean {
            ...
            findTopLevelLogical(text, "or", "||")?.let { (at, token) -> return evaluate(text.substring(0, at)) || evaluate(text.substring(at + token.length)) }
            findTopLevelLogical(text, "and", "&&")?.let { (at, token) -> return evaluate(text.substring(0, at)) && evaluate(text.substring(at + token.length)) }
            val operators = listOf(">=", "<=", "==", "!=", ">", "<")
            for (op in operators) {
                val at = findTopLevelOperator(text, op)
                if (at >= 0) {
                    val a = Expression(text.substring(0, at), env).value(); val b = Expression(text.substring(at + op.length), env).value()
                    return when (op) { ">=" -> a >= b; "<=" -> a <= b; "==" -> abs(a - b) < 1e-9; "!=" -> abs(a - b) >= 1e-9; ">" -> a > b; else -> a < b }
                }
            }
        ...
        private fun stripOuterParentheses(value: String): String {
            var result = value
            while (result.length >= 2 && result.first() == '(' && result.last() == ')' && wrapsWholeExpression(result)) result = result.substring(1, result.lastIndex).trim()
            return result
        }
        ...
        private fun wrapsWholeExpression(value: String): Boolean {
            ...
            value.forEachIndexed { index, ch ->
                when (ch) { '(' -> depth++; ')' -> { depth--; if (depth == 0 && index != value.lastIndex) return false; if (depth < 0) return false } }
            }
            return depth == 0
        }
            ...
            while (i < text.length) {
                ...
                if (depth == 0) {
                    ...
                    if (text.regionMatches(i, word, 0, word.length, ignoreCase = true)) {
                        val beforeOk = i == 0 || text[i - 1].isWhitespace(); val after = i + word.length; val afterOk = after == text.length || text[after].isWhitespace()
        ...
        private fun findTopLevelOperator(text: String, op: String): Int {
            var depth = 0; var i = 0
            while (i <= text.length - op.length) { when (text[i]) { '(' -> depth++; ')' -> depth-- }; if (depth == 0 && text.startsWith(op, i)) return i; i++ }
            return -1
        }
    }
    ...
    private class Expression(private val text: String, private val env: Map<String, Double>) {
        private var i = 0
        fun value(): Double { val result = expression(); skipSpaces(); if (i != text.length) error("Bad expression near '${text.substring(i)}'"); if (!result.isFinite()) error("Expression produced a non-finite number"); return result }
        private fun expression(): Double { var value = term(); while (true) { skipSpaces(); value = when { take('+') -> value + term(); take('-') -> value - term(); else -> return value } } }
        private fun term(): Double { var value = unary(); while (true) { skipSpaces(); value = when { take('*') -> value * unary(); take('/') -> value / unary(); take('%') -> value % unary(); else -> return value } } }
        private fun unary(): Double { skipSpaces(); return when { take('+') -> unary(); take('-') -> -unary(); else -> primary() } }
        private fun primary(): Double {
            skipSpaces(); if (take('(')) { val value = expression(); skipSpaces(); if (!take(')')) error("Missing )"); return value }
            if (i < text.length && (text[i].isDigit() || text[i] == '.')) return number()
            val name = identifier(); skipSpaces(); if (take('(')) { val arg = expression(); skipSpaces(); if (!take(')')) error("Missing ) after $name"); return function(name, arg) }
            return env[name] ?: error("Unknown variable: $name")
        }
        private fun number(): Double { val start = i; while (i < text.length && (text[i].isDigit() || text[i] == '.')) i++; return text.substring(start, i).toDoubleOrNull() ?: error("Invalid number") }
        private fun identifier(): String { skipSpaces(); val start = i; if (i >= text.length || !(text[i].isLetter() || text[i] == '_')) error("Expected a number or variable"); i++; while (i < text.length && (text[i].isLetterOrDigit() || text[i] == '_')) i++; return text.substring(start, i) }
        private fun function(name: String, value: Double): Double = when (name.lowercase()) { "sin" -> sin(Math.toRadians(value)); "cos" -> cos(Math.toRadians(value)); "tan" -> tan(Math.toRadians(value)); "sqrt" -> sqrt(value); "abs" -> abs(value); else -> error("Unknown function: $name") }
        private fun skipSpaces() { while (i < text.length && text[i].isWhitespace()) i++ }
        private fun take(ch: Char): Boolean { if (i < text.length && text[i] == ch) { i++; return true }; return false }
    }
}
